              ...
              <div className="mt-2 text-sm space-y-2">
                ...
                <div className="space-y-1">
                  <div className="flex justify-between">
                    ...
                    <span>{result.positive !== undefined ? `${(result.positive * 100).toFixed(0)}%` : "-"}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-400">Neutral</span>
                    <span>{result.neutral !== undefined ? `${(result.neutral * 100).toFixed(0)}%` : "-"}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-red-400">Negative</span>
                    <span>{result.negative !== undefined ? `${(result.negative * 100).toFixed(0)}%` : "-"}</span>
                  </div>
                </div>
                  ...
                  <div className="mt-3 space-y-1 border-t border-gray-600 pt-2">
                    {result.aspects.map((a, i) => (
                      <div key={i} className="flex justify-between">
                        <span>{a.aspect}</span>
                        <span className={a.sentiment === "positive" ? "text-green-400" : a.sentiment === "negative" ? "text-red-400" : "text-gray-400"}>
                          {a.sentiment}
                        </span>
                      </div>
                    ))}
